...
namespace AcademiaDigital.Application.UseCases.Payments;
    ...
    TimeProvider timeProvider)
{
    public async Task<PaymentDto> Handle(CreatePaymentCommand command, CancellationToken ct = default)
    {
        ...
        var student = await repository.FindStudentByDniAsync(dni, ct)
            ?? throw new KeyNotFoundException("Student not found for the supplied DNI.");
        ...
        if (debts.Any(debt => debt.StudentId != student.Id))
            throw new InvalidOperationException("Every allocated debt must belong to the student identified by DNI.");
        ...
        {
            PublicId = Guid.NewGuid(), StudentId = student.Id, Student = student,
...
{
    public async Task<IReadOnlyList<PaymentDto>> Handle(long? userId, long? studentId, CancellationToken ct = default)
    {
        var payments = studentId.HasValue
            ? await repository.GetByStudentAsync(studentId.Value, ct)
            : await repository.GetByUserAsync(userId ?? throw new ArgumentException("User is required."), ct);
        return payments.Select(PaymentMappings.Map).ToArray();
    }
}

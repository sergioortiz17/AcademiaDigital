...
public sealed class StudentManagementService(Persistence.AppDbContext db) : IStudentManagementService
{
        ...
        StudentStatus? status, int? academicYear, int? commissionId, int page, int pageSize, CancellationToken ct)
    {
        ...
        if (commissionId.HasValue)
            query = query.Where(x => db.StudentAcademicAssignments.Any(a => a.StudentId == x.Id && a.DivisionId == commissionId));

        var total = await query.CountAsync(ct);
        var rows = await query.OrderBy(x => x.User.LastName).ThenBy(x => x.User.Username)
            .Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(ct);
        var ids = rows.Select(x => x.Id).ToList();
        var assignments = await db.StudentAcademicAssignments.AsNoTracking().Include(x => x.Division)
    ...
    public async Task<StudentListItemDto> UpdateStudentAsync(long id, UpdateStudentRequest r, CancellationToken ct)
    {
        ...
        var assignment = await db.StudentAcademicAssignments.AsNoTracking().Include(x => x.Division)
    ...
    public async Task<StudentCareerDto> AddStudentCareerAsync(long studentId, AddStudentCareerRequest r, CancellationToken ct)
    {
        ...
        var membership = new StudentCareer
        {
            ...
            AdmissionYear = (r.EnrollmentDate ?? DateTime.UtcNow).Year
        };
    ...
    public async Task<IReadOnlyList<DivisionDto>> GetDivisionsAsync(int careerId, int? year, CancellationToken ct)
    {
        var q = db.Divisions.AsNoTracking().Where(x => x.CareerId == careerId && x.IsActive);
        ...
        return await q.OrderByDescending(x => x.AcademicYear).ThenBy(x => x.Code).Select(x =>
            new DivisionDto(x.Id, x.CareerId, x.Code, x.Name, x.AcademicYear, x.YearNumber, x.Shift, x.IsActive)).ToListAsync(ct);
    }

    public async Task<DivisionDto> SaveDivisionAsync(int careerId, int? id, UpsertDivisionRequest r, CancellationToken ct)
    {
        ...
        Division item;
        if (id.HasValue) item = await db.Divisions.SingleOrDefaultAsync(x => x.Id == id && x.CareerId == careerId, ct)
            ?? throw new KeyNotFoundException("Comisión no encontrada.");
        else { item = new Division { CareerId = careerId }; db.Divisions.Add(item); }
        var code = r.Code.Trim();
        if (await db.Divisions.AnyAsync(x => x.Id != item.Id && x.CareerId == careerId &&
    ...
    public async Task DisableDivisionAsync(int careerId, int id, CancellationToken ct)
    {
        var item = await db.Divisions.SingleOrDefaultAsync(x => x.Id == id && x.CareerId == careerId, ct)
            ?? throw new KeyNotFoundException("Comisión no encontrada.");
        if (await db.StudentAcademicAssignments.AnyAsync(x => x.DivisionId == id && x.IsCurrent, ct))
    ...
    public async Task<AcademicAssignmentDto> AssignAcademicAsync(long studentId, CreateAcademicAssignmentRequest r, long actorId, CancellationToken ct)
    {
        return await ExecuteAtomicAsync(async () =>
        {
            ...
            var commission = await db.Divisions.SingleOrDefaultAsync(x => x.Id == r.DivisionId && x.IsActive, ct)
                ?? throw new KeyNotFoundException("Comisión no encontrada.");
            ...
            var assignment = new StudentAcademicAssignment { StudentId = studentId, StudentCareerId = membership.Id,
                CareerId = r.CareerId, StudyPlanId = r.StudyPlanId, DivisionId = r.DivisionId,
            ...
            await db.SaveChangesAsync(ct);
            assignment.Division = commission;
    ...
    {
        await EnsureStudent(id, ct);
        var q = db.StudentAcademicAssignments.AsNoTracking().Include(x => x.Division).Where(x => x.StudentId == id);
        if (year.HasValue) q = q.Where(x => x.AcademicYear == year);
    ...
    private static StudentListItemDto MapStudent(Student s, StudentAcademicAssignment? a) => new(s.Id, s.UserId, s.User.Dni,
        ...
        a?.AcademicYear, a?.YearNumber, a?.DivisionId, a?.Division?.Name);
    ...
    private static DivisionDto Map(Division x) => new(x.Id, x.CareerId, x.Code, x.Name, x.AcademicYear, x.YearNumber, x.Shift, x.IsActive);
    private static AcademicAssignmentDto Map(StudentAcademicAssignment x) => new(x.Id, x.StudentId, x.CareerId, x.StudyPlanId,
        x.DivisionId, x.Division?.Name, x.AcademicYear, x.YearNumber, x.StartedAt, x.EndedAt, x.IsCurrent, x.Reason);

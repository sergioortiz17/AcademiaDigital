...
public sealed class FinanceController(
    ...
    GetStudentDebtsQueryHandler debtsQuery) : ApiControllerBase
{
    [HttpGet("concepts")]
    ...
    [HttpPost("plans/{id:long}/generate")]
    public async Task<IActionResult> Generate(long id, [FromHeader(Name = "Idempotency-Key"), Required] string idempotencyKey, CancellationToken ct)
    {
        var guard = RequireAdmin();
        if (guard is not null) return guard;
        return Ok(await generateDebtsHandler.Handle(new(id, idempotencyKey, CurrentUserId!.Value), ct));
    }

    [HttpGet("debts")]
    public async Task<IActionResult> GetStudentDebts([FromQuery, Range(1, long.MaxValue)] long studentId, CancellationToken ct)
    {
        var guard = RequireAdmin();
        if (guard is not null) return guard;
        return Ok(await debtsQuery.Handle(null, studentId, ct));
    }

    [HttpGet("debts/me")]
    public async Task<IActionResult> GetMyDebts(CancellationToken ct)
    {
        ...
        return Ok(await debtsQuery.Handle(CurrentUserId.Value, null, ct));
    }

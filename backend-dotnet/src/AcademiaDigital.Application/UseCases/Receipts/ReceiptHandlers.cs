...
namespace AcademiaDigital.Application.UseCases.Receipts;
    ...
    TimeProvider timeProvider)
{
    ...
    private static ReceiptSnapshot BuildSnapshot(Payment payment, long actorUserId, DateTime issuedAt)
    {
        ...
        return new ReceiptSnapshot(
            ...
            payment.PublicId,
            payment.StudentId,
            $"{payment.Student.User.Username} {payment.Student.User.LastName}".Trim(),
            payment.Student.User.Dni ?? string.Empty,
...
public sealed record GetReceiptsQuery(long ActorUserId, bool IsAdmin, long? StudentId = null);
...
public sealed class GetReceiptsQueryHandler(IReceiptRepository repository)
{
    public async Task<IReadOnlyList<ReceiptDto>> Handle(GetReceiptsQuery query, CancellationToken ct = default)
    {
        if (query.StudentId.HasValue && !query.IsAdmin)
            throw new ForbiddenException("Only administrators can query another student's receipts.");
        var receipts = query.StudentId.HasValue
            ? await repository.GetByStudentAsync(query.StudentId.Value, ct)
            : await repository.GetByUserAsync(query.ActorUserId, ct);
        return receipts.Select(ReceiptMappings.Map).ToArray();
    }
}

public sealed record GetReceiptQuery(Guid PublicId, long ActorUserId, bool IsAdmin);
...
public sealed class GetReceiptQueryHandler(IReceiptRepository repository)
{
    public async Task<ReceiptDto> Handle(GetReceiptQuery query, CancellationToken ct = default)
    {
        var receipt = await repository.FindByPublicIdAsync(query.PublicId, false, ct)
            ?? throw new KeyNotFoundException("Receipt not found.");
        EnsureOwnership(receipt, query.ActorUserId, query.IsAdmin);
        return ReceiptMappings.Map(receipt);
    }

    internal static void EnsureOwnership(Receipt receipt, long actorUserId, bool isAdmin)
    {
        if (!isAdmin && receipt.Payment.Student.UserId != actorUserId)
            throw new ForbiddenException("The receipt belongs to another student.");
    }
}

public sealed record DownloadReceiptQuery(Guid PublicId, long ActorUserId, bool IsAdmin);
...
public sealed class DownloadReceiptQueryHandler(IReceiptRepository repository, IFileStorage fileStorage)
{
    public async Task<StoredFile> Handle(DownloadReceiptQuery query, CancellationToken ct = default)
    {
        var receipt = await repository.FindByPublicIdAsync(query.PublicId, false, ct)
            ?? throw new KeyNotFoundException("Receipt not found.");
        GetReceiptQueryHandler.EnsureOwnership(receipt, query.ActorUserId, query.IsAdmin);

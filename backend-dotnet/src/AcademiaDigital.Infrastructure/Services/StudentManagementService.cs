...
public sealed class StudentManagementService(Persistence.AppDbContext db) : IStudentManagementService
{
    ...
    public async Task<StatusHistoryDto> ChangeStatusAsync(long id, StudentStatus status, string reason, long actorId, CancellationToken ct)
    {
        var student = await db.Students.SingleOrDefaultAsync(x => x.Id == id, ct)
            ?? throw new KeyNotFoundException("Student not found.");
        if (student.Status == status) throw new InvalidOperationException("Student already has the requested status.");
    ...
    public async Task<StudentRecordDto> GetRecordAsync(long id, CancellationToken ct)
    {
        var s = await db.Students.AsNoTracking().Include(x => x.User).Include(x => x.Career)
            .SingleOrDefaultAsync(x => x.Id == id, ct) ?? throw new KeyNotFoundException("Student not found.");
        var assignment = await db.StudentAcademicAssignments.AsNoTracking().Include(x => x.Commission)
    ...
    public async Task<StudentCareerDto> AddStudentCareerAsync(long studentId, AddStudentCareerRequest r, CancellationToken ct)
    {
        ...
        if (await db.StudentCareers.AnyAsync(x => x.StudentId == studentId && x.CareerId == r.CareerId, ct))
            throw new InvalidOperationException("Student is already enrolled in this career.");
    ...
    {
        if (!await db.Careers.AnyAsync(x => x.Id == careerId, ct)) throw new KeyNotFoundException("Career not found.");
        if (!Shifts.Contains(r.Shift, StringComparer.OrdinalIgnoreCase)) throw new ArgumentException("Invalid shift.");
        Commission item;
        ...
        if (await db.Commissions.AnyAsync(x => x.Id != item.Id && x.CareerId == careerId &&
            x.AcademicYear == r.AcademicYear && x.Code == code, ct)) throw new InvalidOperationException("Commission code already exists.");
    ...
    public async Task DisableCommissionAsync(int careerId, int id, CancellationToken ct)
    {
        ...
        if (await db.StudentAcademicAssignments.AnyAsync(x => x.CommissionId == id && x.IsCurrent, ct))
            throw new InvalidOperationException("Commission has current assignments.");
    ...
    public async Task<AcademicAssignmentDto> AssignAcademicAsync(long studentId, CreateAcademicAssignmentRequest r, long actorId, CancellationToken ct)
    {
        return await ExecuteAtomicAsync(async () =>
        {
            ...
            if (plan.CareerId != r.CareerId || commission.CareerId != r.CareerId ||
                ...
                throw new InvalidOperationException("Career, plan, commission and academic cycle are incompatible.");
    ...
    {
        if (r.ValidFrom.HasValue && r.ValidTo < r.ValidFrom) throw new ArgumentException("Invalid validity range.");
        if (r.CareerId.HasValue && !await db.Careers.AnyAsync(x => x.Id == r.CareerId, ct)) throw new KeyNotFoundException("Career not found.");
        DocumentRequirement x;
        if (id.HasValue) x = await db.DocumentRequirements.FindAsync([id.Value], ct) ?? throw new KeyNotFoundException("Requirement not found.");
        else { x = new(); db.DocumentRequirements.Add(x); }
        var code = r.Code.Trim();
        if (await db.DocumentRequirements.AnyAsync(y => y.Id != x.Id && y.Code == code, ct)) throw new InvalidOperationException("Requirement code already exists.");
    ...
    { var x = await db.DocumentRequirements.FindAsync([id], ct) ?? throw new KeyNotFoundException("Requirement not found."); x.IsActive = false; await db.SaveChangesAsync(ct); }
    ...
    {
        await EnsureStudent(id, ct);
        var req = await db.DocumentRequirements.FindAsync([r.DocumentRequirementId], ct) ?? throw new KeyNotFoundException("Requirement not found.");
        if (!ContentTypes.Contains(r.ContentType)) throw new ArgumentException("Unsupported content type.");
    ...
    public async Task<StudentDocumentDto> ReviewDocumentAsync(long studentId, long documentId, ReviewStudentDocumentRequest r, long actorId, CancellationToken ct)
    {
        ...
        if (x.Status != StudentDocumentStatus.Submitted) throw new InvalidOperationException("Only submitted documents can be reviewed.");
        if (r.Status is not (StudentDocumentStatus.Approved or StudentDocumentStatus.Rejected)) throw new ArgumentException("Review status must be Approved or Rejected.");
        if (r.Status == StudentDocumentStatus.Rejected && string.IsNullOrWhiteSpace(r.Observation)) throw new ArgumentException("Observation is required when rejecting.");
    ...
    public async Task DeleteDocumentAsync(long studentId, long documentId, CancellationToken ct)
    {
        var x = await db.StudentDocuments.SingleOrDefaultAsync(y => y.Id == documentId && y.StudentId == studentId, ct)
            ?? throw new KeyNotFoundException("Document not found.");
        if (x.Status == StudentDocumentStatus.Approved) throw new InvalidOperationException("Approved documents cannot be deleted.");
    ...
    {
        Scholarship x;
        if (id.HasValue) x = await db.Scholarships.FindAsync([id.Value], ct) ?? throw new KeyNotFoundException("Scholarship not found.");
        else { x = new(); db.Scholarships.Add(x); }
        var code = r.Code.Trim();
        if (await db.Scholarships.AnyAsync(y => y.Id != x.Id && y.Code == code, ct)) throw new InvalidOperationException("Scholarship code already exists.");
    ...
    { var x = await db.Scholarships.FindAsync([id], ct) ?? throw new KeyNotFoundException("Scholarship not found."); x.IsActive = false; await db.SaveChangesAsync(ct); }
    ...
    public async Task<StudentScholarshipDto> SaveStudentScholarshipAsync(long studentId, long? id, UpsertStudentScholarshipRequest r, long actorId, CancellationToken ct)
    {
        ...
        if (id.HasValue) x = await db.StudentScholarships.SingleOrDefaultAsync(y => y.Id == id && y.StudentId == studentId, ct)
            ?? throw new KeyNotFoundException("Student scholarship not found.");
    ...
    public async Task RevokeStudentScholarshipAsync(long studentId, long id, long actorId, CancellationToken ct)
    {
        var x = await db.StudentScholarships.SingleOrDefaultAsync(y => y.Id == id && y.StudentId == studentId, ct)
            ?? throw new KeyNotFoundException("Student scholarship not found.");
        if (x.Status == StudentScholarshipStatus.Revoked) throw new InvalidOperationException("Scholarship is already revoked.");
    ...
    {
        if (!Regex.IsMatch(r.Key, "^[a-z][a-z0-9_]*$")) throw new ArgumentException("Invalid custom field key.");
        if (r.DataType == CustomFieldDataType.Select && (r.Options is null || r.Options.Count == 0)) throw new ArgumentException("Select fields require options.");
        CustomFieldDefinition x;
        if (id.HasValue) x = await db.CustomFieldDefinitions.FindAsync([id.Value], ct) ?? throw new KeyNotFoundException("Custom field not found.");
        ...
        if (x.Id != 0 && x.DataType != r.DataType && await db.StudentCustomFieldValues.AnyAsync(v => v.CustomFieldDefinitionId == x.Id, ct))
            throw new InvalidOperationException("Data type cannot change after values exist.");
        if (await db.CustomFieldDefinitions.AnyAsync(y => y.Id != x.Id && y.Key == r.Key, ct)) throw new InvalidOperationException("Custom field key already exists.");
    ...
    { var x = await db.CustomFieldDefinitions.FindAsync([id], ct) ?? throw new KeyNotFoundException("Custom field not found."); x.IsActive = false; await db.SaveChangesAsync(ct); }
    ...
    public async Task<IReadOnlyDictionary<string, object?>> SaveCustomValuesAsync(long id, UpsertCustomValuesRequest r, long actorId, CancellationToken ct)
    {
        ...
        if (defs.Count != keys.Count) throw new ArgumentException("One or more custom fields do not exist.");
    ...
    { if (!await db.Students.AnyAsync(x => x.Id == id, ct)) throw new KeyNotFoundException("Student not found."); }
    private static void ValidatePage(ref int page, ref int size)
    { if (page < 1 || size < 1 || size > 100) throw new ArgumentException("Invalid pagination."); }
    ...
    {
        if (raw is null) { if (def.IsRequired) throw new ArgumentException($"{def.Key} is required."); return null; }
        ...
        return def.DataType switch
        {
            ...
            _ => throw new ArgumentException($"Invalid value for {def.Key}.")
        };
    }

...
namespace AcademiaDigital.Finance.Application.UseCases.Finance;
...
public sealed record StudentDebtSummaryDto(long StudentId, decimal TotalOwed, int OverdueCount);
...
// Contract command: the caller (monolith) supplies the student target explicitly.
public sealed record GenerateStudentDebtsCommand(
    long BillingPlanId,
    long StudentId,
    int CareerId,
...
public sealed class UpsertFinancialRateCommandHandler(IFinanceRepository repository, FinancePolicy policy, IUnitOfWork unitOfWork, TimeProvider timeProvider)
{
    public async Task<FinancialRateDto> Handle(UpsertFinancialRateCommand command, CancellationToken ct = default)
    {
        ...
        // Career is owned by the monolith after extraction; CareerId is accepted as an
        // opaque reference (no local FK / existence check possible).
...
public sealed class CreateFinancialBenefitCommandHandler(IFinanceRepository repository, FinancePolicy policy, IUnitOfWork unitOfWork, TimeProvider timeProvider)
{
    public async Task<FinancialBenefitDto> Handle(CreateFinancialBenefitCommand command, CancellationToken ct = default)
    {
        ...
        // CareerId / ScholarshipId are opaque references owned by the monolito after
        // extraction — no local existence check possible.
        var now = timeProvider.GetUtcNow().UtcDateTime;
...
public sealed class GenerateStudentDebtsCommandHandler(IFinanceRepository repository, FinancePolicy policy, IUnitOfWork unitOfWork, TimeProvider timeProvider)
{
    public Task<DebtGenerationResultDto> Handle(GenerateStudentDebtsCommand command, CancellationToken ct = default)
    {
        ...
        var target = new FinanceStudentTarget(
            command.StudentId, command.StudentCareerId, command.Condition,
            string.IsNullOrWhiteSpace(command.StudentCareerId.ToString()) ? string.Empty : $"Alumno {command.StudentId}",
            string.Empty, string.Empty, $"Carrera {command.CareerId}",
            command.GrantedScholarshipIds);
        return unitOfWork.ExecuteInSerializableTransactionAsync(async transactionCt =>
        {
            ...
            if (await repository.StudentCareerHasDebtsForPlanAsync(plan.Id, target.StudentCareerId, transactionCt))
                throw new InvalidOperationException("Debts were already generated for this student career and billing plan.");
            var now = timeProvider.GetUtcNow().UtcDateTime;
...
{
    public async Task<IReadOnlyList<StudentDebtDto>> Handle(long studentId, CancellationToken ct = default)
        => (await repository.GetDebtsByStudentAsync(studentId, ct)).Select(FinanceMappings.Map).ToArray();
}

public sealed class GetStudentDebtSummaryQueryHandler(IFinanceRepository repository)
{
    public async Task<StudentDebtSummaryDto> Handle(long studentId, CancellationToken ct = default)
    {
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
        var debts = await repository.GetDebtsByStudentAsync(studentId, ct);
        var owed = decimal.Round(
            debts.Where(debt => debt.Status is StudentDebtStatus.Pending or StudentDebtStatus.PartiallyPaid)
                 .Sum(debt => debt.TotalAmount - debt.PaidAmount),
            2, MidpointRounding.AwayFromZero);
        var overdue = debts.Count(debt =>
            debt.Status is StudentDebtStatus.Pending or StudentDebtStatus.PartiallyPaid && debt.DueDate < today);
        return new StudentDebtSummaryDto(studentId, owed, overdue);
    }
}
